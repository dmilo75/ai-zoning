...
# Setup text splitter
treshold = 1000 #Rough number of tokens to limit text chunks at
...
def summarize(text):
    ...
    sys_info = "You will be given a section from a municipal zoning ordinance. Your job is to briefly summarize what type of content is in the section in one paragraph. Your summary will be used by other large language models to help determine whether this section is relevant to a specific question about zoning regulation. Remember to keep the summary short."
    response = cg.gpt_api_wrapper(cg.gpt_api_call, (text, "gpt-3.5-turbo-1106", None, None, sys_info, []))
...
def summarize_tree(tree):
    """
    Summarize the text in a hierarchical tree structure.

    :param tree: The hierarchical dictionary or a list of dictionaries representing the tree.
    :return: The same tree structure with summaries added at each node.
    """

    def process_node(node):
        if is_leaf(node):  # If the node is a leaf
            node["Summary"] = summarize(node["Leaf_Text"])
        else:
            # If the node is a branch
            if 'children' in node:
                # Recursively summarize each child
                for child in node['children']:
                    process_node(child)

                # Combine summaries of child nodes for the branch node
                combined_summary = ' '.join([child["Summary"] for child in node['children'] if 'Summary' in child])
                node["Summary"] = summarize(combined_summary)
...
def embed_text(tree):
    ...
    def process_node(node):
        if is_leaf(node):  # Check if the node is a leaf
            ...
            text_list = split_embed_text(text_to_embed)  # Split the text if needed
...
def filter_for_training(file_paths, training_data):
    # Convert training_data Series to a set for efficient lookup
    training_set = set(training_data)

    # Adjust the file paths to match the format of the training data
    adjusted_file_paths = []
    for path in file_paths:
        filename = os.path.basename(path).split('.')[0]
        # Extract relevant parts from the filename to match the training data format
        parts = filename.split('#')
        if len(parts) >= 3:
            adjusted_name = f"{parts[0]}#{parts[1]}#{parts[2]}"
            if adjusted_name in training_set:
                adjusted_file_paths.append(path)
    return adjusted_file_paths
...
def get_files(type = 'all', sample = None):
    ...
    '''
    Could filter for just source is Ordinance.com or for CENSUS_ID_PID6 column in the subset that needs rescraping 
    '''
    ...
    # Option to filter for training data
    if type == 'training':
        training_file = os.path.join(config['raw_data'], "training.pickle")
        with open(training_file, "rb") as fIn:
            training = pickle.load(fIn)
        file_paths = filter_for_training(file_paths, training)
        print(len(file_paths))

    elif type == 'testing':
        testing_file = os.path.join(config['raw_data'], "testing.pickle")
        with open(testing_file, "rb") as fIn:
            testing = pickle.load(fIn)
        file_paths = filter_for_training(file_paths, testing)
        print(len(file_paths))

    elif type == 'wharton':
        wharton_file = os.path.join(config['raw_data'], "wharton_training.pkl")
        with open(wharton_file, "rb") as fIn:
            wharton = pickle.load(fIn)
        file_paths = filter_for_training(file_paths, wharton)
        print(len(file_paths))

    # When we only need a small sample for testing
    if sample is not None:
        random.seed(42)
        file_paths = random.sample(file_paths, sample)

    else:
        #Randomly shuffle order of filepaths with fixed seed
        random.seed(2)
...
for file_path in files_for_task:
    ...
    if os.path.exists(os.path.join(pre_embed_path,file_extension)) and skip_text_process:
        ...
        tree_split = split_tree(cleaned_tree, treshold)
    ...
    #Fifth, create iterative summaries of the text
    '''
    print("Summarizing")
    summ_tree = summarize_tree(embed_tree)
