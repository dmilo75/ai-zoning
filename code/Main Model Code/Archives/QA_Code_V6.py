...
if os.name == 'nt':
    ...
    if len(sys.argv) < 3:
        print("Error: No export folder name or context length provided.")
        sys.exit(1)

    export_folder_name = sys.argv[1]
    context_length = sys.argv[2]
    ...
    os.environ['CONTEXT_LENGTH'] = str(context_length)
    if task_id:
...
munis_for_task = hf.get_munis('testing')

munis_for_task = munis_for_task[0:1]
...
#questions = questions.iloc[0:1]

questions = questions.iloc[[0]]

#questions = questions.loc[[9,13]]
...
if __name__ == "__main__":
    ...
    '''
    Should add something that if status check needed false then terminate program
    '''
