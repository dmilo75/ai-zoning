...
def get_subtasks(question):
    ...
    for i, subtask in hf.subtasks_df.iterrows():
...
class QuestionMuniPair:
    ...
    def create_llm_parsing_object(self, answer,location=0):
        ...
        # Get question string
        question_string = self.question['Question Detail']
    ...
    def create_question_openai_object(self, context, sys_info):
        ...
        if self.question['Question Type'] in ['Binary', 'Lot Size']:
            question_background = hf.load_or_fetch_question_details(self.question)
            ...
            try:
                prompt['Background Information on Question'] = hf.load_or_fetch_question_details(self.question)
