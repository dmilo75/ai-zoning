...
def plot_map_for_size(figsize, gdf_states, fontsize, col):
    ...
    # Create main axes for continental US, occupying most of the figure
    ax_main = fig.add_axes([0.01, 0.2, 1, 0.75])
    ...
    cax = fig.add_axes([0.1, 0.15, 0.8, 0.03])  # [left, bottom, width, height]
    ...
    ax_ak = fig.add_axes([0.15, 0.18, 0.25, 0.25], anchor='SW', axes_class=plt.Axes)
    ax_hi = fig.add_axes([0.5, 0.18, 0.15, 0.15], anchor='SW', axes_class=plt.Axes)
...
for col in get_label.keys():
    ...
    plot_map_for_size((5,4), gdf_states, fontsize, col)
