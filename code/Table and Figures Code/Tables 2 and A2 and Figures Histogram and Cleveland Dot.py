...
population_weighted_mean = (sums['Answer'] * sums['POPULATION']).groupby(sums['Question']).sum() / sums.groupby('Question')['POPULATION'].sum()
...
df_27mean = df[(df['Question'] == 27) & (df['Type'] == 'Mean')][['Answer', 'POPULATION', 'Income Category', 'Urban Category']]
df_27min = df[(df['Question'] == 27) & (df['Type'] == 'Min')][['Answer', 'POPULATION', 'Income Category', 'Urban Category']]
...
for var in ['Median Home Value (Owner Occupied)_2021','Median Gross Rent_2021','Median Home Value (Owner Occupied)_Percent_Change','Median Gross Rent_Percent_Change','Single Unit Permits','Multi-Unit Permits','All Unit Permits']:
...
def calculate_correlation(df,graph =False):
    ...
    for col in cols_to_corr:
        correlations[col] = {}
        for num in questions[questions['Question Type'].isin(['Binary','Numerical'])]['ID'].tolist() + ['27min', '27mean']:
...
def plot_correlations(correlations, sort_by_column=None, save_filename=None):
    ...
    ax = sns.stripplot(data=df_melted, x='Correlation', y='Question', hue='Metric', jitter=False, size=20, alpha = 0.5, order=sort_order, palette=palette)
...
plot_correlations(correlations.copy().drop(columns = ['Median Gross Rent_2021','Median Home Value (Owner Occupied)_2021','All Unit Permits per capita']),
