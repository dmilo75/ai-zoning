...
full_params = {
    ...
    'Chicago': {'abbrev':['il', 'in'],'fips':['17', '18'],'lat':41.8781,'long':-87.6298, 'loc': 'upper right'}, #Since Chicago is so close to Indiana we map Indiana as well
...
for city, params in full_params.items():
    for question, question_text in question_map.items():
        ...
        filtered_data = [
            ...
            if item['Muni'].split('#')[0]+"#"+item['Muni'].split('#')[1] in valid_pairs and item.get('Question') == question
        ]
        ...
        gdf_municipalities_with_text = gdf_municipalities[gdf_municipalities['PLACEFP'].astype(int).isin(all_fips_from_files)]
        gdf_municipalities_without_text = gdf_municipalities[~gdf_municipalities['PLACEFP'].astype(int).isin(all_fips_from_files)]
        ...
        missing_fips = gdf_municipalities_with_text[~gdf_municipalities_with_text['PLACEFP'].astype(int).astype(str).isin(muni_answer_map.keys())]
        ...
        gdf_municipalities_with_text = gdf_municipalities_with_text[gdf_municipalities_with_text['PLACEFP'].astype(int).astype(str).isin(muni_answer_map.keys())]
        ...
        # Conditions and splits for numerical questions
            ...
            if not gdf_municipalities_with_text[condition_q1].empty:
                data_dict['First Quartile'] = {'data': gdf_municipalities_with_text[condition_q1], 'color': '#FFEBEB'}  # Very light red
            if not gdf_municipalities_with_text[condition_q2].empty:
                data_dict['Second Quartile'] = {'data': gdf_municipalities_with_text[condition_q2], 'color': '#FFA5A5'}  # Light red
            if not gdf_municipalities_with_text[condition_q3].empty:
                data_dict['Third Quartile'] = {'data': gdf_municipalities_with_text[condition_q3], 'color': '#FF4C4C'}  # Bright red
            if not gdf_municipalities_with_text[condition_q4].empty:
                data_dict['Fourth Quartile'] = {'data': gdf_municipalities_with_text[condition_q4], 'color': '#8B0000'}  # Dark red
